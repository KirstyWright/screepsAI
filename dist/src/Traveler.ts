...
export class Traveler {
    ...
    const ret = Game.map.findRoute(origin, destination, {
      routeCallback: (roomName: string) => {
        ...
        if (options.preferHighway) {
          ...
          if (parsed) {
            const isHighway = Number(parsed[1]) % 10 === 0 || Number(parsed[2]) % 10 === 0;
            if (isHighway) {
              return 1;
            }
          }
        ...
        if (!options.allowSK && !Game.rooms[roomName]) {
          ...
          const fMod = Number(parsed[1]) % 10;
          const sMod = Number(parsed[2]) % 10;
  ...
  private static deserializeState(travelData: TravelData, destination: RoomPosition): TravelState {
    ...
    if (travelData.state) {
      const serialized = travelData.state;
      state.lastCoord = { x: serialized[STATE_PREV_X], y: serialized[STATE_PREV_Y] };
      state.cpu = serialized[STATE_CPU];
      state.stuckCount = serialized[STATE_STUCK];
      state.destination = new RoomPosition(
        serialized[STATE_DEST_X],
        serialized[STATE_DEST_Y],
        serialized[STATE_DEST_ROOMNAME]
      );
    } else {
      state.cpu = 0;
      state.destination = destination;
