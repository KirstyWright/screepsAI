...
export class Directive {
  ...
  init() {
    const args: string[] = this.flag.name.split("-");
    ...
    if (args.length < 2) {
      ...
      if (typeof id === "number" && Tick.managers[id] != undefined && this.specificDirective != null) {
        const flagName = this.manager.id + "-" + this.flag.pos.roomName + "-" + this.specificDirective.type;
        const pos = this.flag.pos;
        if (Game.rooms[pos.roomName]) {
          const c1 = this.flag.color;
          const c2 = this.flag.secondaryColor;
          this.flag.remove();
