export class RoleMiner {
  ...
  static mine(creep: Creep) {
    const sources = creep.manager.memory.sources;
    if (creep.memory.sourceId) {
      let cachedSource: ManagerMemorySources | null = null;
      let cachedSourceKey: number | null = null;
    for (const key in sources) {
      if (sources[key].sourceId !== creep.memory.sourceId) {
        continue;
      }
      cachedSource = sources[key];
      cachedSourceKey = Number(key);
    }
      if (!cachedSource || cachedSourceKey === null) {
        return;
      }
      if (cachedSource.pos.roomName === creep.pos.roomName) {
        ...
        if (!container && creep.pos.inRangeTo(target.pos.x, target.pos.y, 1)) {
          // Need a container
          const containerList = target.room.find<StructureContainer>(FIND_STRUCTURES, {
            filter: structure => {
              return structure.structureType === STRUCTURE_CONTAINER && structure.pos.inRangeTo(target.pos, 1);
            }
          });
          ...
          const list = target.room.find(FIND_MY_CONSTRUCTION_SITES, {
            filter: structure => {
              return structure.structureType === STRUCTURE_CONTAINER && structure.pos.inRangeTo(target.pos, 1);
            }
          });
