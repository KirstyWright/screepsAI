export class RoleMilitia {
  ...
  static attackTarget(creep: Creep) {
    ...
    const target = Game.getObjectById<Creep | PowerCreep | Structure>(creep.memory.targetId);
    if (target) {
  ...
  static findInManagerRoomsAndAttack(creep: Creep) {
    if (!creep.memory.targetId) {
      ...
      if (hostile) {
        ...
        const hostiles = creep.manager.findInRooms(FIND_HOSTILE_CREEPS);
