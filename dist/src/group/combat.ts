...
export class GroupCombat extends Group {
    ...
    this.creeps.forEach(creep => {
      if (creep.memory.role == "militia") {
        ...
        if (creep.room.name != this.targetRoom) {
          ...
          const attackMove = creep.rangedAttack(target);
          if (attackMove == OK) {
            const path = PathFinder.search(
              creep.pos,
              creep.room.find(FIND_HOSTILE_CREEPS).map(c => {
            ...
            const anotherTarget = creep.pos.findClosestByRange(FIND_HOSTILE_CREEPS);
        ...
        const creepsToHeal = this.creeps.filter(targetCreep => targetCreep.hits < targetCreep.hitsMax);
  ...
  static buildFromMemory(memoryRecord: ManagerMemoryGroup): GroupCombat | false {
    const targetRoom = memoryRecord.targetRoom;
    if (!targetRoom || !memoryRecord.stage) {
      return false;
    }

    return new GroupCombat(memoryRecord.stage, targetRoom, memoryRecord.recruitWhenEnded ?? false, {
