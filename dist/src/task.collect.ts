...
export class CollectTask extends Task {
  ...
  static buildFromMemory(memoryRecord: ManagerMemoryTask): CollectTask | false {
    if (!memoryRecord.target || !memoryRecord.destination || memoryRecord.amount === undefined) {
      return false;
    }
    const target = Game.getObjectById(memoryRecord.target) as Structure | Tombstone | Resource | Ruin | null;
    const destination = Game.getObjectById(memoryRecord.destination) as Structure | null;

    if (!target || !target.id || !destination) {
      return false;
    }

    const task = new CollectTask(target, destination, memoryRecord.amount);
    return task;
  }
  ...
  isValid(): boolean {
    ...
    if (
      (this.target as StructureContainer | Ruin | Tombstone).store !== undefined &&
      (this.target as StructureContainer | Ruin | Tombstone).store.getUsedCapacity(RESOURCE_ENERGY) <= 0
    ) {
    ...
    if (
      (this.destination as StructureContainer | Ruin | Tombstone).store !== undefined &&
      (this.destination as StructureContainer | Ruin | Tombstone).store.getFreeCapacity(RESOURCE_ENERGY) == 0
    ) {
